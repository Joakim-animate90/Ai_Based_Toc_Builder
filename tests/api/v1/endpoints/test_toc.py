...
def mock_toc_service():
    ...
    mock_service.extract_toc_from_upload.return_value = ("Sample TOC content", "toc/table_of_contents.txt")
    return mock_service
...
def mock_pdf_file():
    ...
    # Create a mock file with some content that looks like a PDF
...
@pytest.mark.asyncio
async def test_extract_toc_direct_success(mock_toc_service, valid_toc_request, mock_pdf_file):
    """Test the extract_toc function directly."""
    # Setup
    background_tasks = MagicMock()
    
    # Call function
    result = await extract_toc(mock_pdf_file, valid_toc_request, background_tasks, mock_toc_service)
...
async def test_extract_toc_invalid_file_type(valid_toc_request):
    ...
    with pytest.raises(HTTPException) as excinfo:
        await extract_toc(invalid_file, valid_toc_request, background_tasks, MagicMock())
    
    assert excinfo.value.status_code == 400
    assert "Invalid file" in excinfo.value.detail

@pytest.mark.asyncio
async def test_extract_toc_direct_service_error(mock_toc_service, valid_toc_request, mock_pdf_file):
    ...
    # Call and verify exception
    with pytest.raises(HTTPException) as excinfo:
        await extract_toc(mock_pdf_file, valid_toc_request, background_tasks, mock_toc_service)
